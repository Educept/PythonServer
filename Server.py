...
def generateDerivativeQuestion():
    ...
    eng1 = matlab.engine.connect_matlab('love')
    DQuestions = []
    question = ['f = sqrt(x) - (1/(sqrt(x)));' , 'f = sqrt(x)*(x^2+1);' , 'f = 1/(7*x);' , 'f = (5*x^2)-(5*sqrt(x)-(3/x));', 'f = (3*x-2)*(2*x+1);', 'f = (5*x*sqrt(x))/2 + (1/2*sqrt(x));', 'f = 2/sqrt(x) + sqrt(x)/2' , 'f = -7/(1-x^3)', 'f = 12*x^2-6*x+2' , 'f = 2*x/3 + (6/x^3)' , 'f = sqrt(x) - 1/sqrt(x)']
    # result = {}
    # question = DQuestions()
    # for counter in range(0, 4):
    counter = random.randrange(0,9);
